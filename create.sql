...
-- nedelja, 10. decembar 2017. 17:07:56 cet
...
create table if not exists `formula4000`.`formula` (
  ...
  `kers` varchar(45) null,
  `dvostruki_difuzor` varchar(45) null,
  `f_vod` varchar(45) null,
  `motor` int not null,
  `tim` int not null,
...
use `formula4000`$$
create definer = current_user trigger `formula4000`.`formula_before_update` before update on `formula` for each row
begin
    if new.motor in (select motor from formula) then
		signal sqlstate '45000' set message_text='motor je vec pridruzen nekoj formuli!';
	end if;
...
use `formula4000`$$
drop trigger if exists `formula4000`.`trka_after_insert` $$
use `formula4000`$$
create trigger `formula4000`.`trka_after_insert` after insert on `trka` for each row
begin
	set pobednicki_tim = null;
end$$
